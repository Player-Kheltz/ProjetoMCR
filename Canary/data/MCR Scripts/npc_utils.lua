...
function NpcUtils.correspondeAcao(mensagem, listaSinonimos)
    ...
    for _, sinonimo in ipairs(listaSinonimos) do
        ...
        if mensagem:find(padrao) or
           mensagem:find('^' .. sinonimo:lower() .. '$') or
           mensagem:find('^' .. sinonimo:lower() .. '[%p%c%z%s]') or
           mensagem:find('[%p%c%z%s]' .. sinonimo:lower() .. '$') then
            return true
        end
    end
    return false
...
-- Listas de frases padrão para reuso em outros NPCs
