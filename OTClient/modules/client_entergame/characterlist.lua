...
function CharacterList.init()
    ...
    if G.characters then
        CharacterList.create(G.characters, G.characterAccount)
                -- Dentro da função CharacterList.create, após o loop de criação dos personagens:
            if #characters == 0 then
                local createButton = g_ui.createWidget('Button', charactersWindow)
                createButton:setId('createCharacterButton')
                createButton:setText(tr('Criar Personagem'))
                createButton:addAnchor(AnchorBottom, 'parent', AnchorBottom)
                createButton:addAnchor(AnchorHorizontalCenter, 'parent', AnchorHorizontalCenter)
                createButton:setMarginBottom(20)
                createButton:setWidth(140)
                createButton:setHeight(30)
                createButton.onClick = CharacterList.createCharacter
            end   
    end
end
...
-- ========== FUNÇÃO PARA CRIAR PERSONAGEM (SALÃO DOS DESTINOS) ==========
function CharacterList.createCharacter()
    ...
    local dummyChar = {
        worldHost = G.host,
        worldPort = G.port,
        worldName = "MCR", -- Ajuste para o nome do seu mundo
        characterName = "CriadorDeHerois"
    }

    CharacterList.hide()
    g_game.loginWorld(G.account, G.password, dummyChar.worldName, dummyChar.worldHost, dummyChar.worldPort,
                      dummyChar.characterName, G.authenticatorToken, G.sessionKey)

    loadBox = displayCancelBox(tr('Please wait'), tr('Conectando ao Salão dos Destinos...'))
    connect(loadBox, {
        onCancel = function()
            loadBox = nil
            g_game.cancelLogin()
